...
): { error: ErrorType; passed: boolean } {
...
export const arrToObj: <T extends { _id?: string }>(
  arr: Array<T>,
) => { [key: string]: T } = <T extends { _id?: string }>(arr: Array<T>) => {
...
export const objToArr: <T>(obj: { [key: string]: T }) => T[] = <T>(obj: {
  [key: string]: T;
}) => {
...
export const addColumnAvatar: (
  ...
  width: number,
  height: number,
) => void = (data: UserProps, width: number, height: number) => {
  if (data.avatar) {
    data.avatar.url =
      data.avatar.url +
