...
export interface GlobalDataProps {
  ...
  posts: {
    ...
    loadedColumns: ListProps<{ currentPage: number; total: number }>;
    total: number;
  };
  user: UserProps;
  loading: boolean;
...
function imagePolyfill<T extends WithAvatarProps>(
  list: T[],
  key: 'avatar' | 'image',
  width: number,
  height: number,
  ...
  mutations: {
    ...
    fetchColumns(state, rawData) {
      state.columns = {
        data: {
          ...state.columns.data,
          ...arrToObj(imagePolyfill(rawData.data.list, 'avatar', 50, 50)),
        },
    ...
    fetchColumn(state, rawData) {
      state.columns.data[rawData.data._id] = imagePolyfill(
        [rawData.data],
        'avatar',
        100,
        100,
    ...
    fetchPosts(state, { data: rawData, extraData: columnId }) {
      state.posts.data = {
        ...state.posts.data,
        ...arrToObj(imagePolyfill(rawData.data.list, 'image', 200, 110)),
      };
    ...
    fetchPosts({ state, commit }, { cid, currentPage, pageSize }) {
      if (
        (state.posts.loadedColumns[cid] &&
          state.posts.loadedColumns[cid].currentPage < currentPage) ||
        !state.posts.loadedColumns[cid]
      ) {
        return asyncAndCommit(
          `/api/columns/${cid}/posts?currentPage=${currentPage}&pageSize=${pageSize}`,
          'fetchPosts',
          commit,
          {
            method: 'get',
